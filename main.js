...
const { discover, sweepDiscover, queryOne, LightDevice } = require('./src/light');
...
let store = null;
let quitting = false;
const devices = new Map(); // id -> LightDevice
...
function trayIcon(on) {
    return nativeImage.createFromPath(path.join(__dirname, 'assets', on ? 'tray.png' : 'tray-off.png'));
}
...
ipcMain.handle('devices:rename', (event, id, name) => {
    if (!win || event.sender !== win.webContents || event.senderFrame !== win.webContents.mainFrame) {
        throw new Error('invalid sender');
    }
    if (typeof id !== 'string' || typeof name !== 'string') throw new Error('invalid name');
    const trimmed = name.trim();
    if (!trimmed || trimmed.length > 64 || /[\u0000-\u001f\u007f]/u.test(trimmed)) throw new Error('invalid name');
    const dev = devices.get(id);
