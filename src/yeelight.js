...
function ssdpSearch(targets, timeoutMs) {
  return new Promise((resolve) => {
    ...
    socket.bind(() => {
      ...
      setTimeout(() => { if (!done) sendAll(); }, Math.min(1000, timeoutMs / 2));
...
class YeelightDevice extends EventEmitter {
  ...
  async refresh() {
    ...
    const params = {};
    keys.forEach((k, i) => { if (result[i] !== '') params[k] = result[i]; });
...
module.exports = { discover, sweepDiscover, queryOne, YeelightDevice };
