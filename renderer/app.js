...
function renderDeviceManager() {
    ...
    for (const dev of state.devices) {
        ...
        chk.type = 'checkbox';
        chk.checked = enabled;
        chk.setAttribute('aria-label', deviceLabel(dev));
        ...
        rename.type = 'button';
        rename.className = 'icon-btn rename-btn';
        rename.textContent = '✎';
        rename.title = t('renameDevice');
        rename.setAttribute('aria-label', t('renameDevice') + ': ' + deviceLabel(dev));
        rename.addEventListener('click', () => {
            renameDeviceId = dev.id;
            $('deviceNameInput').value = deviceLabel(dev);
            $('renameStatus').textContent = '';
            $('renameForm').classList.remove('hidden');
            $('deviceNameInput').focus();
            $('deviceNameInput').select();
        });
        row.append(chk, name, ip, rename);
...
for (const event of ['change', 'blur']) {
    $('ctSlider').addEventListener(event, () => {
        ctEditingId = null;
    });
}
...
$('renameForm').addEventListener('submit', async (event) => {
    ...
    const id = renameDeviceId;
    if (!id || $('renameSaveBtn').disabled) return;
    const name = $('deviceNameInput').value.trim();
    if (!name || name.length > 64 || /[\u0000-\u001f\u007f]/u.test(name)) {
        $('renameStatus').textContent = t('invalidName');
        return;
    }
    $('renameSaveBtn').disabled = true;
    $('renameStatus').textContent = '';
    try {
        const dev = await window.gadget.renameDevice(id, name);
        const index = state.devices.findIndex((d) => d.id === dev.id);
        if (index >= 0) state.devices[index] = dev;
        if (renameDeviceId === id) {
            renameDeviceId = null;
            $('renameForm').classList.add('hidden');
        }
        renderAll();
        ...
        if (renameDeviceId === id) $('renameStatus').textContent = t('renameFailed');
    } finally {
